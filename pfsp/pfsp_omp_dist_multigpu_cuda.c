/*
Distributed multi-core multi-GPU B&B to solve Taillard instances of the PFSP in C+MPI+OpenMP+CUDA.
*/
                 ...
                 double *all_timeGpuCpy, double *all_timeGpuMalloc, double *all_timeGpuKer, double *all_timeGenChild, double *all_timePoolOps, double *all_timeGpuIdle, double *all_timeTermination, double *timeLoadBal)
{
  ...
  */
  ...
  startTime = omp_get_wtime();
  int nbThreads = (w != 0) ? NB_THREADS_MAX : NB_THREADS_COMPUTE;
    ...
    reduction(min : best_l) reduction(+ : tree, sol)
  {
    ...
    if (gpuID != NB_THREADS_COMPUTE) // gpuID == D does not manage a GPU!!!
    {
      ...
      if (gpuID % NB_THREADS_GPU == 0)
        cudaSetDevice(gpuID / NB_THREADS_GPU);
    ...
    if (gpuID != NB_THREADS_COMPUTE)
      roundRobin_distribution(pool_loc, &pool_lloc, gpuID, NB_THREADS_COMPUTE);
#pragma omp barrier
    pool_lloc.front = 0;
    pool_lloc.size = 0;
...
#pragma omp barrier
    ...
    while (1)
    {
      ...
      if (gpuID == NB_THREADS_COMPUTE && w == 1)
      {
      ...
      if (gpuID == NB_THREADS_COMPUTE && w == 2)
      {
        startLoadBal = omp_get_wtime();
        ...
        if (!global_termination_flag)
        {
          ...
          if (needs_work && recvNodesSize > 0)
          {
            ...
            for (int l = 0; l < NB_THREADS_COMPUTE; l++)
              atomic_store(&eachTaskState[0l], BUSY);
            local_need = 0;
            ...
            if (allIdle(eachTaskState, NB_THREADS_COMPUTE, &allTasksIdleFlag))
            {
              ...
              if (w == 2)
              {
                ...
                while (atomic_load(&allTasksIdleFlag))
                {
                  ...
                  // break;
                  // if (local_need == 0 && !atomic_load(allTasksIdleFlag))
                  // {
                  //   // request = 0;
                  //   break;
                  // }
                  ...
                  {
                    printf("[%.2f] Proc[%d] Thread[%d] Stille Idle\n", now - cpuIdleStart, MPIRank, gpuID);
                    lastPrintTime = now;
                  }
                  usleep(50);
                  if (global_termination_flag)
...
int main(int argc, char *argv[])
{
  ...
  if (MPIRank == 0)
  {
    ...
    print_results_file_dist_multi_gpu(inst, lb, NB_THREADS_MAX, LB, commSize, optimum, m, M, exploredTree, exploredSol, elapsedTime,
                                      all_expTreeGPU, all_expSolGPU, all_genChildGPU, all_nbStealsGPU, all_nbSStealsGPU, all_nbTerminationGPU, nbSDistLoadBal,
                                      all_timeGpuCpy, all_timeGpuMalloc, all_timeGpuKer, all_timeGenChild, all_timePoolOps, all_timeGpuIdle, all_timeTermination, timeLoadBal);
  }
  if (MPIRank == 0)
