...
if (MPIRank == 0) {
  ...
  */
  ...
  _Atomic bool allTasksIdleFlag = false; //HERE TASKS ARE LOCALE TASKS
    ...
    if (poolSize > 0) {
      ...
      */
      generate_children(parents, poolSize, jobs, bounds, 
        &eachLocaleExploredTree, &eachLocaleExploredSol, &best_l, &pool_lloc);
    }
    else {
      ...
      // local work stealing
      int tries = 0;
      bool steal = false; //TO DO: turn this into remoteSteal

      if (steal == false) {
        // termination
        if (taskState == BUSY) {
          taskState = IDLE;
          atomic_store(&eachTaskState[0], IDLE);
        }
