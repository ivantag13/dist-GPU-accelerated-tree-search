                 ...
                 unsigned long long int *exploredTree, unsigned long long int *exploredSol, double *elapsedTime)
{
  ...
  // Timer
  struct timespec start, startMeasure, end, endMeasure;
  ...
  */
  ...
  double time1, time2, cpyParents = 0, cpyKernel = 0, cpyBounds = 0;
  ...
  while (1)
  {
    ...
    if (poolSize >= m)
    {
      ...
      */
      ...
      // time1 = omp_get_wtime();
      clock_gettime(CLOCK_MONOTONIC_RAW, &startMeasure);
      // time1 = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
      cudaMemcpy(parents_d, parents, poolSize * sizeof(Node), cudaMemcpyHostToDevice);
      // time2 = omp_get_wtime();
      clock_gettime(CLOCK_MONOTONIC_RAW, &endMeasure);
      // cpyParents += time2-time1;
      cpyParents += (endMeasure.tv_sec - startMeasure.tv_sec) + (endMeasure.tv_nsec - startMeasure.tv_nsec) / 1e9;
      ...
      //time2 = omp_get_wtime();
      clock_gettime(CLOCK_MONOTONIC_RAW, &endMeasure);
      cpyKernel += (endMeasure.tv_sec - startMeasure.tv_sec) + (endMeasure.tv_nsec - startMeasure.tv_nsec) / 1e9;
      ...
      clock_gettime(CLOCK_MONOTONIC_RAW, &endMeasure);
      //time2 = omp_get_wtime();
      cpyBounds += (endMeasure.tv_sec - startMeasure.tv_sec) + (endMeasure.tv_nsec - startMeasure.tv_nsec) / 1e9;
  ...
  printf("cpyParents = %.4f, cpyKernel = %.4f, cpyBounds = %.4f\n", cpyParents, cpyKernel, cpyBounds);
